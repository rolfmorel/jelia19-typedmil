...
gen_instance(Input,Output):-
  ...
  findall(SubList,(
    between(1,NumRows,_),
    random(3,4,NumColumns),
    findall(C,(between(1,NumColumns,_),random(65,91,N),random(0,2,Q),M is N + Q*32,char_code(X,M),format(atom(C),"'~w'",[X])),SubList)),Input),
  gen_droplast(Input,Output).
...
print_pos_ex_typed_asp(N):-
  findall(M,(between(1,N,M),gen_instance(X,Y),
  to_asp(X,X1),to_asp(Y,Y1),format("pos_ex_t(f,(list(list(char)),list(list(char))),~w,~w).\n",[X1,Y1])),_).
...
print_neg_ex_asp_both(N):-
  ...
  to_asp(X,X1),to_asp(Y,Y1),
  format("neg_ex(f,~w,~w).\n",[X1,Y1]),format("neg_ex_t(f,(list(list(char)),list(list(char))),~w,~w).\n",[X1,Y1])),_).
