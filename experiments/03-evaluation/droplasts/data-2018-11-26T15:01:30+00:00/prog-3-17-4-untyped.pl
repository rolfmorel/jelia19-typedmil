:- use_module('../../metagol').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P,Q,R],([P,A,B] :- [[Q,A,C],[R,C,B]])).
%metarule(dident,[P,Q,R],([P,A,B] :- [[Q,A,B],[R,A,B]])).
metarule(tohigherorder,[P,Q,F],([P,A,B] :- [[Q,A,B,F]])).
%metarule(tailrec,[P,Q],([P,A,B]:-[[Q,A,C],[P,C,B]])).
my_tail0([_|TL],TL).
my_reverse1(A,B):-reverse(A,B).

map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_last3(A,B):-last(A,B).
my_pred4(A,B):-succ(B,A),A > 0.
my_sumlist5(A,B):-sumlist(A,B).
my_succ6(A,B):-succ(A,B),B =< 10.
my_head7([H|_],H).

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).

my_uppercase9(A):-upcase_atom(A,A).
my_lowercase10(A):-downcase_atom(A,A).
my_flatten11(A,B):-flatten(A,B).
my_even12(A):-0 is A mod 2.
my_toupper13(A,B):-upcase_atom(A,B).
my_msort14(A,B):-msort(A,B).
my_len15(A,B):-length(A,B).
my_tolower16(A,B):-downcase_atom(A,B).
my_min_list17(A,B):-min_list(A,B).
my_element18(A,B):-member(B,A).
my_list_to_set19(A,B):-list_to_set(A,B).
prim(my_tail0/2).
prim(my_reverse1/2).
prim(my_last3/2).
prim(my_pred4/2).
prim(my_sumlist5/2).
prim(my_succ6/2).
prim(my_head7/2).
prim(my_uppercase9/1).
prim(my_lowercase10/1).
prim(my_flatten11/2).
prim(my_even12/1).
prim(my_toupper13/2).
prim(my_msort14/2).
prim(my_len15/2).
prim(my_tolower16/2).
prim(my_min_list17/2).
prim(my_element18/2).
prim(my_list_to_set19/2).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learn(Pos,Neg,H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,False\n").
p([['X','G','s','n'],['O','p','z']],[['X','G','s'],['O','p']]).
p([['Y','i','N','e'],['G','V','a'],['m','l','t','m'],['H','b','J','B']],[['Y','i','N'],['G','V'],['m','l','t'],['H','b','J']]).
p([['T','K','O','k'],['u','P','b'],['c','G','k','w'],['m','N','r','y']],[['T','K','O'],['u','P'],['c','G','k'],['m','N','r']]).
p([['O','F','X'],['n','Z','w']],[['O','F'],['n','Z']]).
p([['Y','D','T'],['l','T','C'],['O','v','J'],['u','O','o']],[['Y','D'],['l','T'],['O','v'],['u','O']]).
q([['u','n','J','P'],['V','g','D','G']],[['u','n','J','P'],['V','g','D']]).
q([['t','l','d'],['s','L','g']],[['t','l','d'],['s','L']]).
q([['D','D','R'],['u','b','W','u'],['f','n','d']],[['D','D','R'],['u','b','W'],['f','n','d']]).
q([['Z','e','j'],['O','Z','Z','g']],[['Z','e','j'],['O','Z','Z']]).
q([['x','A','Z','b'],['I','C','P','E'],['L','w','c','S']],[['x','A','Z','b'],['I','C','P','E'],['L','w','c']]).
