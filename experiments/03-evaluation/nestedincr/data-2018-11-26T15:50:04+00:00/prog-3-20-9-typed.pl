:- use_module('../../metagol-typed').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P:[Ta,Tb],Q:[Ta,Tc],R:[Tc,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Tc],[R,C,B]:[Tc,Tb]])).
%metarule(dident,[P:[Ta,Tb],Q:[Ta,Tb],R:[Ta,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,B]:[Ta,Tb],[R,A,B]:[Ta,Tb]])).
metarule(tohigherorder,[P:[Ta,Tb],Q:[Ta,Tb,Tf],F:Tf],([P,A,B]:[Ta,Tb] :- [[Q,A,B,F]:[Ta,Tb,Tf]])).
%metarule(tailrec,[P:[Ta,Tb],Q:[Ta,Ta]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Ta],[P,C,B]:[Ta,Tb]])).

map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_succ1(A,B):-succ(A,B),B =< 10.
my_flatten2(A,B):-flatten(A,B).

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).

my_set4(A):-list_to_set(A,A).
my_tolower5(A,B):-downcase_atom(A,B),char_code(A,_).
my_min_list6(A,B):-min_list(A,B).
my_pred7(A,B):-succ(B,A),A > 0.
my_len8(A,B):-length(A,B).
my_toupper9(A,B):-upcase_atom(A,B),char_code(A,_).
my_even10(A):-0 is A mod 2.
my_lowercase11(A):-downcase_atom(A,A),char_code(A,_).
my_uppercase12(A):-upcase_atom(A,A),char_code(A,_).
my_list_to_set13(A,B):-list_to_set(A,B).
my_max_list14(A,B):-max_list(A,B).
my_reverse15(A,B):-reverse(A,B).
my_element16(A,B):-member(B,A).
my_double17(N,M):-M is 2*N,M =< 10.
my_sumlist18(A,B):-sumlist(A,B).
my_head19([H|_],H).
my_odd20(A):-1 is A mod 2.
my_last21(A,B):-last(A,B).
prim(my_succ1,[int,int]).
prim(my_flatten2,[list(list(T)),list(T)]).
prim(my_set4,[list(_)]).
prim(my_tolower5,[char,char]).
prim(my_min_list6,[list(int),int]).
prim(my_pred7,[int,int]).
prim(my_len8,[list(_),int]).
prim(my_toupper9,[char,char]).
prim(my_even10,[int]).
prim(my_lowercase11,[char]).
prim(my_uppercase12,[char]).
prim(my_list_to_set13,[list(T),list(T)]).
prim(my_max_list14,[list(int),int]).
prim(my_reverse15,[list(T),list(T)]).
prim(my_element16,[list(T),T]).
prim(my_double17,[int,int]).
prim(my_sumlist18,[list(int),int]).
prim(my_head19,[list(T),T]).
prim(my_odd20,[int]).
prim(my_last21,[list(T),T]).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learntyped(Pos,Neg,[list(list(int)),list(list(int))],H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,True\n").
p([[7,5,3],[7,5,5,5],[5,5,3,0],[7,4,0]],[[9,7,5],[9,7,7,7],[7,7,5,2],[9,6,2]]).
p([[5,0,7],[4,6,7,4]],[[7,2,9],[6,8,9,6]]).
p([[1,7,5],[5,6,1],[4,5,1,6]],[[3,9,7],[7,8,3],[6,7,3,8]]).
p([[7,2,0],[6,3,3]],[[9,4,2],[8,5,5]]).
p([[2,7,5,2],[1,1,2]],[[4,9,7,4],[3,3,4]]).
q([[3,4,1,1],[3,6,7]],[[5,6,3,3],[3,6,7]]).
q([[6,3,0],[3,1,2,2],[1,2,6],[7,2,0,2]],[[6,3,0],[5,3,4,4],[3,4,8],[9,4,2,4]]).
q([[7,7,1],[7,4,4],[2,6,5]],[[9,9,3],[7,4,4],[4,8,7]]).
q([[5,6,0],[3,1,7,3],[2,3,6,0]],[[7,8,2],[5,3,9,5],[2,3,6,0]]).
q([[3,6,1,0],[2,4,0,7],[1,2,5,4],[4,5,7]],[[3,6,1,0],[4,6,2,9],[3,4,7,6],[6,7,9]]).
