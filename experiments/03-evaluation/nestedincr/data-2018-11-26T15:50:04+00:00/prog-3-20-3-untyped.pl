:- use_module('../../metagol').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P,Q,R],([P,A,B] :- [[Q,A,C],[R,C,B]])).
%metarule(dident,[P,Q,R],([P,A,B] :- [[Q,A,B],[R,A,B]])).
metarule(tohigherorder,[P,Q,F],([P,A,B] :- [[Q,A,B,F]])).
%metarule(tailrec,[P,Q],([P,A,B]:-[[Q,A,C],[P,C,B]])).

map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_succ1(A,B):-succ(A,B),B =< 10.
my_lowercase2(A):-downcase_atom(A,A),char_code(A,_).
my_tail3([_|TL],TL).
my_list_to_set4(A,B):-list_to_set(A,B).
my_even5(A):-0 is A mod 2.
my_max_list6(A,B):-max_list(A,B).
my_reverse7(A,B):-reverse(A,B).
my_len8(A,B):-length(A,B).
my_sumlist9(A,B):-sumlist(A,B).

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).

my_min_list11(A,B):-min_list(A,B).
my_set12(A):-list_to_set(A,A).
my_pred13(A,B):-succ(B,A),A > 0.
my_flatten14(A,B):-flatten(A,B).
my_msort15(A,B):-msort(A,B).
my_odd16(A):-1 is A mod 2.
my_double17(N,M):-M is 2*N,M =< 10.
my_toupper18(A,B):-upcase_atom(A,B),char_code(A,_).
my_last19(A,B):-last(A,B).
my_uppercase20(A):-upcase_atom(A,A),char_code(A,_).
my_tolower21(A,B):-downcase_atom(A,B),char_code(A,_).
prim(my_succ1/2).
prim(my_lowercase2/1).
prim(my_tail3/2).
prim(my_list_to_set4/2).
prim(my_even5/1).
prim(my_max_list6/2).
prim(my_reverse7/2).
prim(my_len8/2).
prim(my_sumlist9/2).
prim(my_min_list11/2).
prim(my_set12/1).
prim(my_pred13/2).
prim(my_flatten14/2).
prim(my_msort15/2).
prim(my_odd16/1).
prim(my_double17/2).
prim(my_toupper18/2).
prim(my_last19/2).
prim(my_uppercase20/1).
prim(my_tolower21/2).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learn(Pos,Neg,H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,False\n").
p([[1,4,4],[6,4,1,6]],[[3,6,6],[8,6,3,8]]).
p([[1,4,0,5],[1,2,7]],[[3,6,2,7],[3,4,9]]).
p([[7,6,7],[1,6,3,4]],[[9,8,9],[3,8,5,6]]).
p([[3,4,1,5],[1,3,5,4],[2,1,4,5],[6,0,1]],[[5,6,3,7],[3,5,7,6],[4,3,6,7],[8,2,3]]).
p([[3,4,4,1],[7,4,4,6]],[[5,6,6,3],[9,6,6,8]]).
q([[4,2,1,2],[5,4,1]],[[4,2,1,2],[7,6,3]]).
q([[5,2,1,2],[5,5,7]],[[5,2,1,2],[7,7,9]]).
q([[2,5,1],[1,0,6,3],[2,1,6]],[[4,7,3],[3,2,8,5],[2,1,6]]).
q([[7,1,0,0],[2,2,0,7],[0,4,2,4]],[[9,3,2,2],[2,2,0,7],[2,6,4,6]]).
q([[6,0,5],[5,5,6,3]],[[8,2,7],[5,5,6,3]]).
