:- use_module('../../metagol-typed').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P:[Ta,Tb],Q:[Ta,Tc],R:[Tc,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Tc],[R,C,B]:[Tc,Tb]])).
%metarule(dident,[P:[Ta,Tb],Q:[Ta,Tb],R:[Ta,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,B]:[Ta,Tb],[R,A,B]:[Ta,Tb]])).
metarule(tohigherorder,[P:[Ta,Tb],Q:[Ta,Tb,Tf],F:Tf],([P,A,B]:[Ta,Tb] :- [[Q,A,B,F]:[Ta,Tb,Tf]])).
%metarule(tailrec,[P:[Ta,Tb],Q:[Ta,Ta]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Ta],[P,C,B]:[Ta,Tb]])).

map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_succ1(A,B):-succ(A,B),B =< 10.
my_uppercase2(A):-upcase_atom(A,A),char_code(A,_).
my_min_list3(A,B):-min_list(A,B).
my_head4([H|_],H).
my_last5(A,B):-last(A,B).
my_double6(N,M):-M is 2*N,M =< 10.
my_odd7(A):-1 is A mod 2.
my_set8(A):-list_to_set(A,A).
my_reverse9(A,B):-reverse(A,B).
my_tolower10(A,B):-downcase_atom(A,B),char_code(A,_).
my_element11(A,B):-member(B,A).
my_list_to_set12(A,B):-list_to_set(A,B).
my_toupper13(A,B):-upcase_atom(A,B),char_code(A,_).
my_msort14(A,B):-msort(A,B).
my_pred15(A,B):-succ(B,A),A > 0.
my_lowercase16(A):-downcase_atom(A,A),char_code(A,_).
my_even17(A):-0 is A mod 2.

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).

my_sumlist19(A,B):-sumlist(A,B).
my_max_list20(A,B):-max_list(A,B).
my_len21(A,B):-length(A,B).
my_tail22([_|TL],TL).
prim(my_succ1,[int,int]).
prim(my_uppercase2,[char]).
prim(my_min_list3,[list(int),int]).
prim(my_head4,[list(T),T]).
prim(my_last5,[list(T),T]).
prim(my_double6,[int,int]).
prim(my_odd7,[int]).
prim(my_set8,[list(_)]).
prim(my_reverse9,[list(T),list(T)]).
prim(my_tolower10,[char,char]).
prim(my_element11,[list(T),T]).
prim(my_list_to_set12,[list(T),list(T)]).
prim(my_toupper13,[char,char]).
prim(my_msort14,[list(int),list(int)]).
prim(my_pred15,[int,int]).
prim(my_lowercase16,[char]).
prim(my_even17,[int]).
prim(my_sumlist19,[list(int),int]).
prim(my_max_list20,[list(int),int]).
prim(my_len21,[list(_),int]).
prim(my_tail22,[list(T),list(T)]).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learntyped(Pos,Neg,[list(list(int)),list(list(int))],H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,True\n").
p([[7,4,5],[7,5,6,7],[1,2,3,6]],[[9,6,7],[9,7,8,9],[3,4,5,8]]).
p([[3,5,1,2],[7,1,3],[1,5,7,0]],[[5,7,3,4],[9,3,5],[3,7,9,2]]).
p([[3,1,5],[2,2,1,4],[4,3,7]],[[5,3,7],[4,4,3,6],[6,5,9]]).
p([[1,6,1],[2,6,0,2]],[[3,8,3],[4,8,2,4]]).
p([[2,4,3],[7,7,0]],[[4,6,5],[9,9,2]]).
q([[3,4,6,0],[7,6,5,3],[1,4,0,1]],[[5,6,8,2],[9,8,7,5],[1,4,0,1]]).
q([[7,5,7],[2,3,2,6]],[[7,5,7],[4,5,4,8]]).
q([[7,1,6,5],[0,4,0],[3,5,6],[7,0,7]],[[9,3,8,7],[0,4,0],[5,7,8],[9,2,9]]).
q([[4,0,5,4],[3,7,1,2],[4,3,3]],[[6,2,7,6],[3,7,1,2],[6,5,5]]).
q([[3,0,6,1],[5,4,0]],[[3,0,6,1],[7,6,2]]).
