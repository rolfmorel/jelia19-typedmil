:- use_module('../../metagol-typed').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P:[Ta,Tb],Q:[Ta,Tc],R:[Tc,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Tc],[R,C,B]:[Tc,Tb]])).
%metarule(dident,[P:[Ta,Tb],Q:[Ta,Tb],R:[Ta,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,B]:[Ta,Tb],[R,A,B]:[Ta,Tb]])).
metarule(tohigherorder,[P:[Ta,Tb],Q:[Ta,Tb,Tf],F:Tf],([P,A,B]:[Ta,Tb] :- [[Q,A,B,F]:[Ta,Tb,Tf]])).
%metarule(tailrec,[P:[Ta,Tb],Q:[Ta,Ta]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Ta],[P,C,B]:[Ta,Tb]])).

map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_succ1(A,B):-succ(A,B),B =< 10.
my_toupper2(A,B):-upcase_atom(A,B),char_code(A,_).
my_reverse3(A,B):-reverse(A,B).
my_odd4(A):-1 is A mod 2.
my_even5(A):-0 is A mod 2.
my_tail6([_|TL],TL).

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).

my_msort8(A,B):-msort(A,B).
my_tolower9(A,B):-downcase_atom(A,B),char_code(A,_).
my_min_list10(A,B):-min_list(A,B).
my_pred11(A,B):-succ(B,A),A > 0.
my_set12(A):-list_to_set(A,A).
my_last13(A,B):-last(A,B).
my_max_list14(A,B):-max_list(A,B).
prim(my_succ1,[int,int]).
prim(my_toupper2,[char,char]).
prim(my_reverse3,[list(T),list(T)]).
prim(my_odd4,[int]).
prim(my_even5,[int]).
prim(my_tail6,[list(T),list(T)]).
prim(my_msort8,[list(int),list(int)]).
prim(my_tolower9,[char,char]).
prim(my_min_list10,[list(int),int]).
prim(my_pred11,[int,int]).
prim(my_set12,[list(_)]).
prim(my_last13,[list(T),T]).
prim(my_max_list14,[list(int),int]).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learntyped(Pos,Neg,[list(list(int)),list(list(int))],H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,True\n").
p([[4,4,4,1],[2,0,4,7]],[[6,6,6,3],[4,2,6,9]]).
p([[6,0,0,6],[7,3,0],[0,7,5]],[[8,2,2,8],[9,5,2],[2,9,7]]).
p([[7,7,6],[4,6,1,7],[5,7,7,2],[0,7,5,5]],[[9,9,8],[6,8,3,9],[7,9,9,4],[2,9,7,7]]).
p([[4,1,6,3],[0,7,6,6],[1,4,2],[7,2,3]],[[6,3,8,5],[2,9,8,8],[3,6,4],[9,4,5]]).
p([[2,0,1,7],[5,2,6,0],[1,1,6,5]],[[4,2,3,9],[7,4,8,2],[3,3,8,7]]).
q([[3,3,0],[5,4,7,6],[5,6,6,1]],[[5,5,2],[7,6,9,8],[5,6,6,1]]).
q([[6,4,2,6],[5,6,6,1]],[[8,6,4,8],[5,6,6,1]]).
q([[3,7,5,1],[1,5,4,2],[4,3,7]],[[5,9,7,3],[3,7,6,4],[4,3,7]]).
q([[4,0,3],[1,0,2,6]],[[4,0,3],[3,2,4,8]]).
q([[7,0,1,0],[4,0,5]],[[9,2,3,2],[4,0,5]]).
