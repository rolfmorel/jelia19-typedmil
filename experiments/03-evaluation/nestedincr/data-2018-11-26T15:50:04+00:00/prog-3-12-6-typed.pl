:- use_module('../../metagol-typed').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P:[Ta,Tb],Q:[Ta,Tc],R:[Tc,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Tc],[R,C,B]:[Tc,Tb]])).
%metarule(dident,[P:[Ta,Tb],Q:[Ta,Tb],R:[Ta,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,B]:[Ta,Tb],[R,A,B]:[Ta,Tb]])).
metarule(tohigherorder,[P:[Ta,Tb],Q:[Ta,Tb,Tf],F:Tf],([P,A,B]:[Ta,Tb] :- [[Q,A,B,F]:[Ta,Tb,Tf]])).
%metarule(tailrec,[P:[Ta,Tb],Q:[Ta,Ta]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Ta],[P,C,B]:[Ta,Tb]])).

map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_succ1(A,B):-succ(A,B),B =< 10.
my_list_to_set2(A,B):-list_to_set(A,B).
my_odd3(A):-1 is A mod 2.
my_tail4([_|TL],TL).
my_len5(A,B):-length(A,B).
my_last6(A,B):-last(A,B).
my_msort7(A,B):-msort(A,B).
my_lowercase8(A):-downcase_atom(A,A),char_code(A,_).
my_sumlist9(A,B):-sumlist(A,B).
my_head10([H|_],H).
my_even11(A):-0 is A mod 2.
my_min_list12(A,B):-min_list(A,B).
my_set13(A):-list_to_set(A,A).
prim(my_succ1,[int,int]).
prim(my_list_to_set2,[list(T),list(T)]).
prim(my_odd3,[int]).
prim(my_tail4,[list(T),list(T)]).
prim(my_len5,[list(_),int]).
prim(my_last6,[list(T),T]).
prim(my_msort7,[list(int),list(int)]).
prim(my_lowercase8,[char]).
prim(my_sumlist9,[list(int),int]).
prim(my_head10,[list(T),T]).
prim(my_even11,[int]).
prim(my_min_list12,[list(int),int]).
prim(my_set13,[list(_)]).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learntyped(Pos,Neg,[list(list(int)),list(list(int))],H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,True\n").
p([[5,0,2],[3,3,6],[7,6,0]],[[7,2,4],[5,5,8],[9,8,2]]).
p([[1,7,6,4],[1,7,7],[4,4,4]],[[3,9,8,6],[3,9,9],[6,6,6]]).
p([[7,4,3],[0,3,4]],[[9,6,5],[2,5,6]]).
p([[5,3,0,2],[7,0,6,0],[4,6,1,5]],[[7,5,2,4],[9,2,8,2],[6,8,3,7]]).
p([[2,5,6],[0,7,3,6],[2,1,4]],[[4,7,8],[2,9,5,8],[4,3,6]]).
q([[4,4,0,0],[5,3,1]],[[4,4,0,0],[7,5,3]]).
q([[3,1,1],[6,1,5],[1,5,1],[0,4,0]],[[5,3,3],[8,3,7],[1,5,1],[2,6,2]]).
q([[6,4,6],[7,2,0,6],[2,1,6,0]],[[8,6,8],[7,2,0,6],[4,3,8,2]]).
q([[7,3,7,2],[2,2,4,4],[5,7,1],[0,6,4,5]],[[7,3,7,2],[4,4,6,6],[7,9,3],[2,8,6,7]]).
q([[1,1,2],[5,7,2,6],[3,1,3],[2,6,5,1]],[[3,3,4],[7,9,4,8],[5,3,5],[2,6,5,1]]).
