...
uppercase(A):-upcase_atom(A,A),char_code(A,_).
tolower(A,B):-downcase_atom(A,B),char_code(A,_).

gen_filtercapslower(A,B):-gen_filtercapslower1(A,C),gen_filtercapslower2(C,B).
gen_filtercapslower1(A,B):-filter(A,B,uppercase).
gen_filtercapslower2(A,B):-maplist(tolower,A,B).
