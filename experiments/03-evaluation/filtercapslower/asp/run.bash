...
for i in $(seq 0 $PREDS); do
	...
	echo "print_pos_ex_asp_both($EXAMPLES),print_neg_ex_asp_both($EXAMPLES)." | swipl gen_examples.pl 2>/dev/null | grep 'pos_ex\|neg_ex' | sed "s/l(/(/g;s/'/\"/g" | tee -a $DIR/prog-$CLAUSES-$i-$j-untyped.hex >> $DIR/prog-$CLAUSES-$i-$j-typed.hex
    done
done 
