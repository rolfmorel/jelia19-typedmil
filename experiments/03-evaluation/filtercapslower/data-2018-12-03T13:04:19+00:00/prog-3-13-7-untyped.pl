:- use_module('../../metagol').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P,Q,R],([P,A,B] :- [[Q,A,C],[R,C,B]])).
%metarule(dident,[P,Q,R],([P,A,B] :- [[Q,A,B],[R,A,B]])).
metarule(tohigherorder,[P,Q,F],([P,A,B] :- [[Q,A,B,F]])).
%metarule(tailrec,[P,Q],([P,A,B]:-[[Q,A,C],[P,C,B]])).
my_uppercase0(A):-upcase_atom(A,A).
my_tolower1(A,B):-downcase_atom(A,B).

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).


map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_max_list4(A,B):-max_list(A,B).
my_even5(A):-0 is A mod 2.
my_flatten6(A,B):-flatten(A,B).
my_pred7(A,B):-succ(B,A),A > 0.
my_last8(A,B):-last(A,B).
my_list_to_set9(A,B):-list_to_set(A,B).
my_min_list10(A,B):-min_list(A,B).
my_sumlist11(A,B):-sumlist(A,B).
my_head12([H|_],H).
my_odd13(A):-1 is A mod 2.
my_double14(N,M):-M is 2*N,M =< 10.
my_succ15(A,B):-succ(A,B),B =< 10.
my_element16(A,B):-member(B,A).
prim(my_uppercase0/1).
prim(my_tolower1/2).
prim(my_max_list4/2).
prim(my_even5/1).
prim(my_flatten6/2).
prim(my_pred7/2).
prim(my_last8/2).
prim(my_list_to_set9/2).
prim(my_min_list10/2).
prim(my_sumlist11/2).
prim(my_head12/2).
prim(my_odd13/1).
prim(my_double14/2).
prim(my_succ15/2).
prim(my_element16/2).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learn(Pos,Neg,H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,False\n").
p([l,'K',n,e,'L',m,b,'C','W'],[k,l,c,w]).
p(['R','O','F','O'],[r,o,f,o]).
p(['M','W',x,z,'E','S',r,k],[m,w,e,s]).
p(['J',z,p,'E','O','Y','K'],[j,e,o,y,k]).
p([l,'N','I',u,'V'],[n,i,v]).
q(['J',r,'T',i,'C',n,z,'K'],[t,j,j,c,k]).
q(['W','J','I','V','Z',o,p],[j,i,v,z,i,w]).
q(['I',x,p,y,a,l,'R','X'],[x,i,r,'W']).
q([k,'X',m,'B',n,y,w],['F',b,x]).
q(['B',w,q,w,q],[b,l]).
