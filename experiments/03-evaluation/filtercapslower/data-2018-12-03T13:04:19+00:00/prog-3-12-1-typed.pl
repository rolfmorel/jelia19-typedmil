:- use_module('../../metagol-typed').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P:[Ta,Tb],Q:[Ta,Tc],R:[Tc,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Tc],[R,C,B]:[Tc,Tb]])).
%metarule(dident,[P:[Ta,Tb],Q:[Ta,Tb],R:[Ta,Tb]],([P,A,B]:[Ta,Tb] :- [[Q,A,B]:[Ta,Tb],[R,A,B]:[Ta,Tb]])).
metarule(tohigherorder,[P:[Ta,Tb],Q:[Ta,Tb,Tf],F:Tf],([P,A,B]:[Ta,Tb] :- [[Q,A,B,F]:[Ta,Tb,Tf]])).
%metarule(tailrec,[P:[Ta,Tb],Q:[Ta,Ta]],([P,A,B]:[Ta,Tb] :- [[Q,A,C]:[Ta,Ta],[P,C,B]:[Ta,Tb]])).
my_uppercase0(A):-upcase_atom(A,A).
my_tolower1(A,B):-downcase_atom(A,B).

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).


map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_sumlist4(A,B):-sumlist(A,B).
my_double5(N,M):-M is 2*N,M =< 10.
my_toupper6(A,B):-upcase_atom(A,B).
my_odd7(A):-1 is A mod 2.
my_pred8(A,B):-succ(B,A),A > 0.
my_min_list9(A,B):-min_list(A,B).
my_list_to_set10(A,B):-list_to_set(A,B).
my_msort11(A,B):-msort(A,B).
my_element12(A,B):-member(B,A).
my_max_list13(A,B):-max_list(A,B).
my_head14([H|_],H).
my_lowercase15(A):-downcase_atom(A,A).
prim(my_uppercase0,[char]).
prim(my_tolower1,[char,char]).
prim(my_sumlist4,[list(int),int]).
prim(my_double5,[int,int]).
prim(my_toupper6,[char,char]).
prim(my_odd7,[int]).
prim(my_pred8,[int,int]).
prim(my_min_list9,[list(int),int]).
prim(my_list_to_set10,[list(T),list(T)]).
prim(my_msort11,[list(int),list(int)]).
prim(my_element12,[list(T),T]).
prim(my_max_list13,[list(int),int]).
prim(my_head14,[list(T),T]).
prim(my_lowercase15,[char]).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learntyped(Pos,Neg,[list(char),list(char)],H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,True\n").
p(['R','E','O',j,i,'D','G','I',b],[r,e,o,d,g,i]).
p([b,'V',h,'T','Z',q,'C',y,'Y'],[v,t,z,c,y]).
p(['E','H',s,c,k],[e,h]).
p([u,'X','R',f,j,d,'K'],[x,r,k]).
p([p,d,'X',u,g,'T','E',r],[x,t,e]).
q(['P',d,'Z',n,m,'L','O','Z','I'],[g,p,o,z,i,l,z]).
q(['D',e,'Q',z],[q,'M',d]).
q(['L',q,'F','Z',o,'M',m,q,s],[z,f,l,'V',m]).
q([c,'C','X',r,j,'K','I'],[c,k,x,i,'A']).
q([m,b,s,w,'A'],[a,o]).
