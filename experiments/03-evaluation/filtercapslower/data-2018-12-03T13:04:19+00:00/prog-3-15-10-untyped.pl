:- use_module('../../metagol').
:- use_module(library(system)).
:- use_module(library(lists)).
metagol:max_clauses(3).



metarule(chain,[P,Q,R],([P,A,B] :- [[Q,A,C],[R,C,B]])).
%metarule(dident,[P,Q,R],([P,A,B] :- [[Q,A,B],[R,A,B]])).
metarule(tohigherorder,[P,Q,F],([P,A,B] :- [[Q,A,B,F]])).
%metarule(tailrec,[P,Q],([P,A,B]:-[[Q,A,C],[P,C,B]])).
my_uppercase0(A):-upcase_atom(A,A).
my_tolower1(A,B):-downcase_atom(A,B).

filter([],[],_F).
filter([A|T1],[A|T2],F):-
  call(F,A),
  filter(T1,T2,F).
filter([_|T1],T2,F):-
  filter(T1,T2,F).
interpreted(filter/3).

inter(filter_base,([filter,[],[],_]:[list(T),list(T),[T]]:-[])).
inter(filter_ind_incl,([filter,[H1|T1],[H1|T2],F]:[list(T),list(T),[T]]:-[[F,H1]:[T],[filter,T1,T2,F]:[list(T),list(T),[T]]])).
inter(filter_ind_excl,([filter,[_|T1],T2,F]:[list(T),list(T),[T]]:-[[filter,T1,T2,F]:[list(T),list(T),[T]]])).


map([],[],_F).
map([A|As],[B|Bs],F):-
  call(F,A,B),
  map(As,Bs,F).
interpreted(map/3).

inter(map_base,([map,[],[],_]:[list(S),list(T),[S,T]]:-[])).
inter(map_ind,([map,[H1|T1],[H2|T2],F]:[list(S),list(T),[S,T]]:-[[F,H1,H2]:[S,T],[map,T1,T2,F]:[list(S),list(T),[S,T]]])).

my_list_to_set4(A,B):-list_to_set(A,B).
my_msort5(A,B):-msort(A,B).
my_len6(A,B):-length(A,B).
my_last7(A,B):-last(A,B).
my_reverse8(A,B):-reverse(A,B).
my_toupper9(A,B):-upcase_atom(A,B).
my_lowercase10(A):-downcase_atom(A,A).
my_min_list11(A,B):-min_list(A,B).
my_succ12(A,B):-succ(A,B),B =< 10.
my_double13(N,M):-M is 2*N,M =< 10.
my_set14(A):-list_to_set(A,A).
my_sumlist15(A,B):-sumlist(A,B).
my_element16(A,B):-member(B,A).
my_max_list17(A,B):-max_list(A,B).
my_pred18(A,B):-succ(B,A),A > 0.
prim(my_uppercase0/1).
prim(my_tolower1/2).
prim(my_list_to_set4/2).
prim(my_msort5/2).
prim(my_len6/2).
prim(my_last7/2).
prim(my_reverse8/2).
prim(my_toupper9/2).
prim(my_lowercase10/1).
prim(my_min_list11/2).
prim(my_succ12/2).
prim(my_double13/2).
prim(my_set14/1).
prim(my_sumlist15/2).
prim(my_element16/2).
prim(my_max_list17/2).
prim(my_pred18/2).
run :-get_time(T1),
  MaxTime=600, % 10 min
  findall(p(A,B),(p(A,B)),Pos),
  findall(p(A,B),(q(A,B)),Neg),
  catch(call_with_time_limit(MaxTime, (learn(Pos,Neg,H);true)),
      time_limit_exceeded,
      H = no_answer),
%  time_out((;true),MaxTime,Result),
  get_time(T2),
  Duration is T2-T1,
  pprint(H),
  format('%data,time,~f\n',[Duration]),
  format("%data,num_clauses,3\n"),
  format("%data,types_enabled,False\n").
p([u,'W',j,c,'I','J'],[w,i,j]).
p([l,'R',h,z,'F','M','E',v,'D'],[r,f,m,e,d]).
p(['Y','O','F',f,'B','S',f,t],[y,o,f,b,s]).
p(['F',v,'M','N'],[f,m,n]).
p(['C',s,'P',u,p,'F','U',k,u],[c,p,f,u]).
q(['Q',k,l,'Y'],[y,'T',q]).
q([b,'C',z,w,'F','J','X'],[j,c,o,x,f]).
q(['Y',g,'J',m,'M'],[m,j,y,'J']).
q(['J',v,q,'V','X','P',m,'Q',g],[v,'E',q,p,x,j]).
q(['C',k,'Y','R','Y','J',u],[c,y,e,r,y,j]).
