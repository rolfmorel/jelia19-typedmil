...
:- module(metagol,[learn/2,learn/3,learntyped/3,learntyped/4,learn_seq/2,pprint/1,op(950,fx,'@')]).
...
learntyped(Pos1,Neg1,Type):-
    learn(Pos1,Neg1,Type,Prog),
    pprint(Prog).

learntyped(Pos1,Neg1,Type,Prog):-
    learn(Pos1,Neg1,Type,Prog).

learn(Pos1,Neg1,Prog):-
    learn(Pos1,Neg1,_,Prog).

learn(Pos1,Neg1,Type,Prog):-
    maplist(atom_to_list,Pos1,Pos2),
    maplist(atom_to_list,Neg1,Neg2),
    maplist(add_type_vars,Pos2,Pos3),
    maplist(add_type_vars,Neg2,Neg3),
    append(Pos3,Neg3,Atoms),
    maplist(dtype,Atoms,DTypes),
    const_list(Type,DTypes),
    %% print(Atoms),
    proveall(Pos3,Sig,Prog),
    nproveall(Neg3,Sig,Prog),
...
prove_aux(p(prim,P,_A,Args,DT,GT,_Atom,_Path),_FullSig,_Sig,_MaxN,N,N,Prog,Prog):-
    %% writeln(prim-Atom),
    %% writeln(DT-GT),
    user:primcall(P,DT,GT,Args).

%% %% disprove a prim
%% prove_aux(p(_,NegP,_A,Args,DT,GT,_Atom,_Path),_FullSig,_Sig,_MaxN,N,N,Prog,Prog):-
%% %% prove_aux(p(_,NegP,_A,Args,_Atom,_Path),_FullSig,_Sig,_MaxN,N,N,Prog,Prog):-
%%     nonvar(NegP),
%%     NegP=not,!,
%%     trace,
%%     Args=[P|PArgs],
%%     \+user:primcall(P,DT,GT,PArgs).
...
prove_aux(p(inv,_P,_A,_Args,DT,GT,Atom,Path),FullSig,Sig,MaxN,N1,N2,Prog1,Prog2):-
    %% inv could be renamed
    writeln(inter-Atom),
    inter(_,(Atom:DT:GT):-Body1,Path),
    %% writeln(Body1),
    %% trace,
    prove(Body1,FullSig,Sig,MaxN,N1,N2,Prog1,Prog2).

%% use existing abduction
prove_aux(p(inv,P,A,Args,DT,GT,Atom,Path),FullSig,Sig1,MaxN,N1,N2,Prog1,Prog2):-
    select_lower(P,A,GT,FullSig,Sig1,Sig2),
    \+(\+(DT=GT)), % verify that derivation type is still an instance of polymorphic type
    member(sub(Name,P,A,MetaSub,PredTypes),Prog1),

    (Name==map->checkmapargs(Args); true),

    user:metarule_init(Name,MetaSub,PredTypes,(Atom:DT:GT:-Body1),Recursive,[Atom|Path]),
    (Recursive==true -> \+memberchk(Atom,Path); true),
    prove(Body1,FullSig,Sig2,MaxN,N1,N2,Prog1,Prog2).

%% new abduction
prove_aux(p(inv,P,A,_Args,DT,GT,Atom,Path),FullSig,Sig1,MaxN,N1,N2,Prog1,Prog2):-
    (N1 == MaxN -> fail; true),
    bind_lower(P,A,GT,FullSig,Sig1,Sig2),
    user:metarule(Name,MetaSub,PredTypes,(Atom:DT:GT :- Body1),FullSig,Recursive,[Atom|Path]),
    \+(\+(DT=GT)), % verify that derivation type is still an instance of polymorphic type
    (Recursive==true -> \+memberchk(Atom,Path); true),
...
checkmapargs([A,B]):-
    true.
    %% ((ground(A),ground(B)) -> (length(A,N),length(B,N)); true).


add_empty_path([P|Args]:DT:GT,p(inv,P,A,Args,DT,GT,[P|Args],[])):- %Q: why both P,...,Args and [P|Args]?
    size(Args,A).

select_lower(P,A,GType,FullSig,_Sig1,Sig2):-
    nonvar(P),!,
    append(_,[sym(P,A,GType,_)|Sig2],FullSig),!.

select_lower(P,A,GType,_FullSig,Sig1,Sig2):-
    append(_,[sym(P,A,GType,U)|Sig2],Sig1),
    (var(U)-> !,fail;true ).

bind_lower(P,A,GType,FullSig,_Sig1,Sig2):-
    nonvar(P),!,
    append(_,[sym(P,A,GType,_)|Sig2],FullSig),!.

bind_lower(P,A,GType,_FullSig,Sig1,Sig2):-
    append(_,[sym(P,A,GType,U)|Sig2],Sig1),
    (var(U)-> U = 1,!;true).
...
target_predicate([[P|Args]:_:_|_],P/A):-
    length(Args,A).

invented_symbols(MaxClauses,P/A,[sym(P,A,_T,_U)|Sig]):-
    NumSymbols is MaxClauses-1,
    get_option(max_inv_preds(MaxInvPreds)),
    M is min(NumSymbols,MaxInvPreds),
    findall(sym(InvSym,_Arity,_Type,_Used),(between(1,M,I),atomic_list_concat([P,'_',I],InvSym)),Sig).
...
pprint(Prog1):-
    ...
    (get_option(unfold_program) -> unfold_program(Prog3,Prog4); Prog3=Prog4), % FIXME: update to handle types
    maplist(remove_orderings,Prog4,Prog5),
    (get_option(print_types) -> Prog5=Prog6; maplist(strip_types,Prog5,Prog6)),
    maplist(clause_list_to_clause,Prog6,Prog7),
    maplist(pprint_clause,Prog7).
...
metasub_to_clause_list(sub(Name,_,_,MetaSub,_),[HeadList|BodyAsList]):-
    user:metarule_init(Name,MetaSub,_,(HeadList:-Body1),_,_),
    add_path_to_body(BodyAsList,_,Body1,_).

list_to_clause([Atom],Atom):-!.
list_to_clause([Atom|T1],(Atom,T2)):-!,
    list_to_clause(T1,T2).

list_to_atom(AtomList:T,Atom:T):-!,
    Atom =..AtomList.
...
user:term_expansion(inter(N,(Head:DT :- BodyDT)),user:inter(N,(Head:DT:GT :- Body2),Path)):-
    copy_term((Head:DT :- BodyDT),(_:GT :- BodyGT)),
    zip_goal_types(BodyDT,BodyGT,Body1),
    add_path_to_body(Body1,Path,Body2,_).


user:term_expansion(Clause,user:inter(N,(Head:DT:GT :- Body2),Path):-Body):-
    Clause = (inter(N,(Head:DT :- BodyDT)):-Body),
    copy_term((Head:DT :- BodyDT),(_:GT :- BodyGT)),
    zip_goal_types(BodyDT,BodyGT,Body1),
    add_path_to_body(Body1,Path,Body2,_).
...
user:term_expansion(prim(P,Type),[user:(primcall(P,Type,GType,Args):-user:Call)]):-
    copy_term(Type,GType),
    size(Type,A),
    functor(Call,P,A),
    Call =.. [P|Args].
...
get_asserts(Name,MetaSub,Clause1,MetaBody,PS,[MRule,MInit]):-
    Clause1 = ([P|Args]:GT :- BodyGT),
    copy_term(Clause1,(_:DT :- BodyDT)),
    is_recursive(BodyGT,P,Recursive),
    zip_goal_types(BodyDT,BodyGT,Body1),
    add_path_to_body(Body1,Path,Body2,PredTypes),
    Clause2 = ([P|Args]:DT:GT :- Body2),
    ...
    %% (var(MetaBody) ->
    MInit = metarule_init(AssertName,MetaSub,PredTypes,Clause2,Recursive,Path).
        %% MInit = (metarule_init(AssertName,MetaSub,PredTypes,Clause2,Recursive,Path) :- MetaBody)).

is_recursive([],_,false).
is_recursive([[Q|_]:_|_],P,true):-
    Q==P,!.
...
add_path_to_body([[P|Args]:DT:GT|Atoms],Path,[p(PType,P,A,Args,DT,GT,[P|Args],Path)|Rest],[PType|Out]):-
    size(Args,A),
    add_path_to_body(Atoms,Path,Rest,Out).
