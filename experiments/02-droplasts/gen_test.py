...
[FO,'my_succ',lambda n: "my_succ{}(A,B):-succ(A,B).".format(n),'/2','[int,int]'],
[FO,'my_pred',lambda n: "my_pred{}(A,B):-succ(B,A).".format(n),'/2','[int,int]'],
...
[FO,'my_double',lambda n: "my_double{}(N,M):-M is 2*N.".format(n),'/2','[int,int]'],
...
%metarule(tohigherorder1,[P:[list(S),list(T)],Q:[list(S),list(T),[S,T]],F:[S,T]],([P,A,B]:[list(S),list(T)] :- [[Q,A,B,F]:[list(S),list(T),[S,T]]])).
%metarule(tohigherorder2,[P:[list(T),list(T)],Q:[list(T),list(T),[T]],F:[T]],([P,A,B]:[list(T),list(T)] :- [[Q,A,B,F]:[list(T),list(T),[T]]])).
""" 
...
#
  ...
  invocation = "learntyped(Pos,[],[list(list(char)),list(list(char))],H)" if types_enabled else "learn(Pos,[],H)"
